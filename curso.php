		...
		<tr>
	    	<th height="100%" valign="top" >
	    		<table width="100%" height="100%" border="1">
					<tr>
			        	<th width="400" height="120" valign="top">
			        		<table>
			        			<br><br>
			        			<?php
					    ...
					    <th width="1000" valign="top" bgcolor="9CC6D8">
					    	<br/><br/>
					    	<?php  
						    	$idPlan = "SELECT idPlan FROM plan WHERE nombre_cur='".$_GET['value']."'";
								$resultado_idPlan=mysql_query($idPlan);
								$row_idPlan = mysql_fetch_array($resultado_idPlan);
								$Plan_id=$row_idPlan['idPlan'];

								//Buscar imagenes por curso
								include('Backend/conexion.php');
								$query_imagen = "select * from imagen where Plan_id=$Plan_id";
								$resultado_imagen = mysqli_query($conn,$query_imagen);
					    	?>
						    ...
						    <div align='center'>
							    <table class='tabl'>
							        <tr>
							        	<?php 
										$i=0;
							        	foreach($resultado_imagen as $item){ ?>
						         			<td><img width="300"  height="300" src="Backend/imagenes/<?php echo $item['nombre']; ?>" ></td>
						  				<?php
						  				$i++;
						  				if($i==3){
						  					$i=0;
